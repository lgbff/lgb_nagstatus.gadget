...
<xsl:template match="/nagstatus">
    ...
    <div id="problist" style="position:absolute;overflow:auto;height:340px;width:485px;top:55px;left:14px;color:white;font-size:xx-small;">
...
<xsl:template match="service">
	<!-- <xsl:if test="@description!='Check_MK inventory'"> -->
		...
		<xsl:if test="current_state&gt;0 and problem_has_been_acknowledged=0 and scheduled_downtime_depth=0 and @description!='Check_MK inventory' and @description!='APT' and @description!='Zypper Updates' and @description!='LOG System' and @description!='LOG Application' and @description!='ESX Guest Tools'">
			...
			Last change: <xsl:value-of select="nagstatus:timestampToString(last_state_change*1000)" /><br />
			<!-- Test: <xsl:value-of select="nagstatus:noServices(@description,'LOG.*')" /> -->
			</div>
		</xsl:if>
	<!-- </xsl:if> -->
</xsl:template>
